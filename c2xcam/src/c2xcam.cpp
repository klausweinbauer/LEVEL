...
int getOctetString(OCTET_STRING_t *octetString, uint8_t *buffer,
                   int bufferSize) {
...
int createCAM(int stationID) {
  ...
  if (it != databaseCAM_.end()) {
    ...
    errMsgStream << "CAM with Id '" << stationID << "' already exists. "
                 << "You can not have two CAMMessage Blocks with the same Id."
                 << std::endl;
...
int setCAMBasicContainer(int stationID, int stationType, int latitude,
                         int longitude, int confidenceMajor,
                         int confidenceMinor, int confidenceMajorOrientation,
...
int getBasicVehicleContainerHighFrequency(
    int stationID, BasicVehicleContainerHighFrequency_t **container,
  ...
  if (cam->cam.camParameters.highFrequencyContainer.present ==
    ...
    cam->cam.camParameters.highFrequencyContainer.present =
        HighFrequencyContainer_PR_basicVehicleContainerHighFrequency;
             ...
             HighFrequencyContainer_PR_basicVehicleContainerHighFrequency) {
    std::stringstream errMsgStream;
    errMsgStream << "Wrong type of HighFrequencyContainer. "
                 << "HighFrequencyContainer of CAMMessage from station '"
                 << stationID
                 << "' was already specified as 'RSUContainerHighFrequency'. "
                 << "You can not use multiple HighFrequencyContainers."
                 << std::endl;
    setLastErrMsg(errMsgStream.str().c_str(), (int)errMsgStream.str().size());
    return ERR_HIGH_FREQ_CONTAINER_TYPE;
  }

  *container = &cam->cam.camParameters.highFrequencyContainer.choice
                    .basicVehicleContainerHighFrequency;
  return 0;
}

int setCAMBasicVehicleContainerHighFrequency(
...
int setCAMBasicVehicleContainerHighFrequencyLanePosition(int stationID,
                                                         int lanePosition) {
  databaseLockCAM_.lock();
  BasicVehicleContainerHighFrequency *hfc;
  int err = getBasicVehicleContainerHighFrequency(stationID, &hfc, true);
...
int setCAMBasicVehicleContainerHighFrequencyLateralAcceleration(
  ...
  hfc->lateralAcceleration->lateralAccelerationValue = lateralAccelerationValue;
...
int setCAMBasicVehicleContainerHighFrequencyPerformanceClass(
...
int setCAMBasicVehicleContainerHighFrequencyCenDsrcTollingZone(
  ...
  hfc->cenDsrcTollingZone->protectedZoneLongitude = protectedZoneLongitude;
...
int setCAMBasicVehicleContainerLowFrequency(int stationID, int vehicleRole,
...
int addCAMBasicVehicleContainerLowFrequencyPathPoint(int stationID,
                                                     int deltaLatitude,
                                                     int deltaLongitude,
                                                     int deltaAltitude,
                                                     int deltaTime) {
...
int getRSUContainerHighFrequency(int stationID,
  ...
  if (cam->cam.camParameters.highFrequencyContainer.present ==
    ...
    std::stringstream errMsgStream;
    errMsgStream
        << "Wrong type of HighFrequencyContainer. HighFrequencyContainer of "
           "CAMMessage from station '"
        << stationID
        << "' was already specified as 'BasicVehicleContainerHighFrequency'. "
           "You can not use multiple HighFrequencyContainers."
        << std::endl;
    setLastErrMsg(errMsgStream.str().c_str(), (int)errMsgStream.str().size());
    return ERR_HIGH_FREQ_CONTAINER_TYPE;
...
int addCAMRSUContainerHighFrequencyProtectedCommunicationZone(
  ...
  ProtectedCommunicationZonesRSU *zonesRSU =
...
int clearCAMRSUContainerHighFrequencyProtectedCommunicationZones(
    int stationID) {
  databaseLockCAM_.lock();
  RSUContainerHighFrequency_t *container;
  int err = getRSUContainerHighFrequency(stationID, &container);
...
int getSpecialVehicleContainer(int stationID, void **container,
                               SpecialVehicleContainer_PR type,
                               bool initialize = false) {
  CAM_t *cam = getCAM(stationID);
  if (!cam) {
    return ERR_MSG_NOT_FOUND;
  ...
  if (cam->cam.camParameters.specialVehicleContainer->present == type) {
    *container = &cam->cam.camParameters.specialVehicleContainer->choice;
    ...
    std::stringstream errMsgStream;
    errMsgStream << "Wrong type of SpecialVehicleContainer. You can only have "
                    "one type of SpecialVehicleContainer per CAM message. "
                 << "Container is of type '"
                 << toString(
...
template <typename T>
int getSpecialVehicleContainer(int stationID, T **container,
                               bool initialize = false);
template <>
int getSpecialVehicleContainer<PublicTransportContainer>(
    int stationID, PublicTransportContainer **container, bool initialize) {
  return getSpecialVehicleContainer(
      stationID, (void **)container,
      SpecialVehicleContainer_PR_publicTransportContainer, initialize);
}
template <>
int getSpecialVehicleContainer<SpecialTransportContainer>(
    int stationID, SpecialTransportContainer **container, bool initialize) {
  return getSpecialVehicleContainer(
      stationID, (void **)container,
      SpecialVehicleContainer_PR_specialTransportContainer, initialize);
}
template <>
int getSpecialVehicleContainer<DangerousGoodsContainer>(
    int stationID, DangerousGoodsContainer **container, bool initialize) {
  return getSpecialVehicleContainer(
      stationID, (void **)container,
      SpecialVehicleContainer_PR_dangerousGoodsContainer, initialize);
}
template <>
int getSpecialVehicleContainer<RoadWorksContainerBasic>(
    int stationID, RoadWorksContainerBasic **container, bool initialize) {
  return getSpecialVehicleContainer(
      stationID, (void **)container,
      SpecialVehicleContainer_PR_roadWorksContainerBasic, initialize);
}
template <>
int getSpecialVehicleContainer<RescueContainer>(int stationID,
                                                RescueContainer **container,
                                                bool initialize) {
  return getSpecialVehicleContainer(stationID, (void **)container,
                                    SpecialVehicleContainer_PR_rescueContainer,
                                    initialize);
}
template <>
int getSpecialVehicleContainer<EmergencyContainer>(
    int stationID, EmergencyContainer **container, bool initialize) {
  return getSpecialVehicleContainer(
      stationID, (void **)container,
      SpecialVehicleContainer_PR_emergencyContainer, initialize);
}
template <>
int getSpecialVehicleContainer<SafetyCarContainer>(
    int stationID, SafetyCarContainer **container, bool initialize) {
  return getSpecialVehicleContainer(
      stationID, (void **)container,
      SpecialVehicleContainer_PR_safetyCarContainer, initialize);
}

int setCAMPublicTransportContainer(int stationID, int embarkationStatus,
  ...
  setOctetString(&c->ptActivation->ptActivationData, ptActivationData,
                 ptActivationDataSize);

  databaseLockCAM_.unlock();
  return 0;
}

int setCAMSpecialTransportContainer(int stationID,
  ...
  setBitString(&c->specialTransportType, specialTransportType,
               specialTransportTypeSize);
  setBitString(&c->lightBarSirenInUse, lightBarSirenInUse,
               lightBarSirenInUseSize);
...
int setCAMRoadWorksContainerBasic(int stationID, int roadworksSubCauseCode,
                                  uint8_t *lightBarSirenInUse,
                                  int lightBarSirenInUseSize,
                                  int closedLaneInnerhardShoulderStatus,
                                  int closedLaneOuterhardShoulderStatus,
                                  uint8_t *closedLaneDrivingLaneStatus,
  ...
  *c->closedLanes->outerhardShoulderStatus = closedLaneOuterhardShoulderStatus;
  setBitString(c->closedLanes->drivingLaneStatus, closedLaneDrivingLaneStatus,
               closedLaneDrivingLaneStatusSize);

  databaseLockCAM_.unlock();
  return 0;
}

int setCAMRescueContainer(int stationID, uint8_t *lightBarSirenInUse,
                          int lightBarSirenInUseSize) {
  databaseLockCAM_.lock();
  RescueContainer *c;
  int err = getSpecialVehicleContainer(stationID, &c, true);
...
int setCAMEmergencyContainer(int stationID, uint8_t *lightBarSirenInUse,
                             int lightBarSirenInUseSize, int causeCode,
                             int subCauseCode, uint8_t *emergencyPriority,
...
int setCAMSafetyCarContainer(int stationID, uint8_t *lightBarSirenInUse,
                             int lightBarSirenInUseSize, int causeCode,
                             int subCauseCode, int trafficRule,
                             int speedLimit) {
  databaseLockCAM_.lock();
...
void getCAMBasicContainer(CAM_t *cam, int *stationType, int *latitude,
                          int *longitude, int *confidenceMajor,
                          int *confidenceMinor, int *confidenceMajorOrientation,
                          int *altitudeValue, int *altitudeConfidence) {
  if (stationType) {
...
int getCAMBasicContainer(int stationID, int *stationType, int *latitude,
                         int *longitude, int *confidenceMajor,
                         int *confidenceMinor, int *confidenceMajorOrientation,
                         int *altitudeValue, int *altitudeConfidence) {
  databaseLockCAM_.lock();
  ...
  getCAMBasicContainer(cam, stationType, latitude, longitude, confidenceMajor,
                       confidenceMinor, confidenceMajorOrientation,
...
int getCAMBasicVehicleContainerHighFrequencyLanePosition(int stationID,
                                                         int *lanePosition) {
  databaseLockCAM_.lock();
  BasicVehicleContainerHighFrequency *hfc;
  int err = getBasicVehicleContainerHighFrequency(stationID, &hfc, true);
...
int getCAMBasicVehicleContainerHighFrequencySteeringWheelAngle(
  ...
  if (steeringWheelAngleValue) {
    *steeringWheelAngleValue = hfc->steeringWheelAngle->steeringWheelAngleValue;
  }
...
int getCAMBasicVehicleContainerHighFrequencyPerformanceClass(
    int stationID, int *performanceClass) {
  databaseLockCAM_.lock();
  BasicVehicleContainerHighFrequency *hfc;
  int err = getBasicVehicleContainerHighFrequency(stationID, &hfc, true);
...
int getCAMBasicVehicleContainerHighFrequencyCenDsrcTollingZone(
  ...
  if (protectedZoneLongitude) {
    *protectedZoneLongitude = hfc->cenDsrcTollingZone->protectedZoneLongitude;
  }
...
int getCAMBasicVehicleContainerLowFrequency(CAM_t *cam, int *vehicleRole,
                                            uint8_t *exteriorLights,
                                            int exteriorLightsSize,
                                            int *actualExteriorLightsSize) {
  if (!cam->cam.camParameters.lowFrequencyContainer) {
    return ERR_NULL;
  }
...
int getCAMBasicVehicleContainerLowFrequency(int stationID, int *vehicleRole,
                                            uint8_t *exteriorLights,
                                            int exteriorLightsSize,
                                            int *actualExteriorLightsSize) {
  databaseLockCAM_.lock();
  ...
  int ret = getCAMBasicVehicleContainerLowFrequency(
      cam, vehicleRole, exteriorLights, exteriorLightsSize,
...
int getCAMBasicVehicleContainerLowFrequencyPathHistory(
    CAM_t *cam, int *pathHistory, int pathHistorySize,
  ...
  if (pathHistory) {
    ...
    for (int i = 0; i < cpySize; i++) {
      pathHistory[i * 4 + 0] =
          lfc->pathHistory.list.array[len - 1 - i]->pathPosition.deltaLatitude;
      pathHistory[i * 4 + 1] =
          lfc->pathHistory.list.array[len - 1 - i]->pathPosition.deltaLongitude;
      pathHistory[i * 4 + 2] =
          lfc->pathHistory.list.array[len - 1 - i]->pathPosition.deltaAltitude;
      pathHistory[i * 4 + 3] =
          *lfc->pathHistory.list.array[len - 1 - i]->pathDeltaTime;
    }
  }
...
int getCAMBasicVehicleContainerLowFrequencyPathHistory(
  ...
  int ret = getCAMBasicVehicleContainerLowFrequencyPathHistory(
      cam, pathHistory, pathHistorySize, actualPathHistorySize);

  databaseLockCAM_.unlock();
  return ret;
}

int getCAMRSUContainerHighFrequencyProtectedCommunicationZone(
  ...
  if (protectedZoneType) {
    *protectedZoneType = zone->protectedZoneType;
  }
  if (expiryTime) {
    getTimestamp(zone->expiryTime, expiryTime);
  }
  if (zone->protectedZoneRadius && protectedZoneRadius) {
    *protectedZoneRadius = *zone->protectedZoneRadius;
  }
  if (zone->protectedZoneID && protectedZoneID) {
    *protectedZoneID = *zone->protectedZoneID;
  }
  if (protectedZoneLatitude) {
    *protectedZoneLatitude = zone->protectedZoneLatitude;
  }
  if (protectedZoneLongitude) {
    *protectedZoneLongitude = zone->protectedZoneLongitude;
  }
...
int getCAMPublicTransportContainer(int stationID, int *embarkationStatus,
                                   int *ptActivationType,
                                   uint8_t *ptActivationData,
                                   int ptActivationDataSize) {
  databaseLockCAM_.lock();
  ...
  if (c->ptActivation) {
    getOctetString(&c->ptActivation->ptActivationData, ptActivationData,
                   ptActivationDataSize);
  }
...
int getCAMSpecialTransportContainer(int stationID,
                                    uint8_t *specialTransportType,
                                    int specialTransportTypeSize,
                                    uint8_t *lightBarSirenInUse,
                                    int lightBarSirenInUseSize) {
  ...
  getBitString(&c->specialTransportType, specialTransportType,
               specialTransportTypeSize);
  getBitString(&c->lightBarSirenInUse, lightBarSirenInUse,
               lightBarSirenInUseSize);
...
int getCAMRoadWorksContainerBasic(int stationID, int *roadworksSubCauseCode,
                                  uint8_t *lightBarSirenInUse,
                                  int lightBarSirenInUseSize,
                                  int *closedLaneInnerhardShoulderStatus,
                                  int *closedLaneOuterhardShoulderStatus,
  ...
  getBitString(&c->lightBarSirenInUse, lightBarSirenInUse,
               lightBarSirenInUseSize);
  if (closedLaneOuterhardShoulderStatus && c->closedLanes &&
      c->closedLanes->outerhardShoulderStatus) {
    *closedLaneOuterhardShoulderStatus =
        *c->closedLanes->outerhardShoulderStatus;
  }
  if (closedLaneInnerhardShoulderStatus && c->closedLanes &&
      c->closedLanes->innerhardShoulderStatus) {
    *closedLaneInnerhardShoulderStatus =
        *c->closedLanes->innerhardShoulderStatus;
  }
  if (closedLaneDrivingLaneStatus && c->closedLanes &&
      c->closedLanes->drivingLaneStatus) {
    getBitString(c->closedLanes->drivingLaneStatus, closedLaneDrivingLaneStatus,
                 closedLaneDrivingLaneStatusSize);
  }
...
int getCAMRescueContainer(int stationID, uint8_t *lightBarSirenInUse,
                          int lightBarSirenInUseSize) {
  databaseLockCAM_.lock();
  RescueContainer *c;
  int err = getSpecialVehicleContainer(stationID, &c);
...
int getCAMEmergencyContainer(int stationID, uint8_t *lightBarSirenInUse,
                             int lightBarSirenInUseSize, int *causeCode,
                             int *subCauseCode, uint8_t *emergencyPriority,
                             int emergencyPrioritySize) {
  databaseLockCAM_.lock();
...
int getCAMSafetyCarContainer(int stationID, uint8_t *lightBarSirenInUse,
                             int lightBarSirenInUseSize, int *causeCode,
                             int *subCauseCode, int *trafficRule,
                             int *speedLimit) {
  databaseLockCAM_.lock();
...
int decodeCAM(int *stationID, uint8_t *buffer, int bufferSize, int encoding) {
  ...
  *stationID = cam->header.stationID;
  ...
  if (cam->cam.camParameters.lowFrequencyContainer) {
    ...
    if (lfc->present ==
        LowFrequencyContainer_PR_basicVehicleContainerLowFrequency) {
      lfc->choice.basicVehicleContainerLowFrequency.pathHistory.list.free =
          freePathPoint;
    }
  }
...
int writeCallbackCAM(const void *src, size_t size,
                     void *application_specific_key) {
  return (int)writeCallbackBufferCAM_->write(src, (int)size,
                                             application_specific_key);
}

int encodeCAM(int stationID, uint8_t *buffer, int bufferSize,
  ...
  switch ((EncodingType)encoding) {
  case XER_BASIC:
    retVal = xer_encode(&asn_DEF_CAM, (void *)cam, XER_F_BASIC,
                        writeCallbackCAM, NULL);
    break;
  case XER_CANONICAL:
  ...
  if (retVal.encoded == -1) {
    ...
    if (xmlTag.compare(xmlTagHighFrequencyContainerString) == 0) {
      ...
      errMsgStream << "CAM Encoding failed (Code=" << retVal.failed_type->name
                   ...
                   << "' in the message of Station '" << stationID << "'."
                   << std::endl;
    }
    setLastErrMsg(errMsgStream.str().c_str(), (int)errMsgStream.str().size());
    return ERR_ENCODE;
...
int setCAMIDsForTransmission(int *stationIDs_send, int numberStationIDs) {
  return CAMTransmitter::getInstance().setIDsToTransmit(stationIDs_send,
                                                        numberStationIDs);
}
