...
public:
  ...
  virtual std::vector<BYTE> encode(const std::shared_ptr<T> message) {

    std::vector<BYTE> buffer;
    asn_enc_rval_t retVal;
    retVal = xer_encode(&_typeDescriptor, (void *)message.get(), _encoding,
                        writeCallback, (void *)&buffer);

    if (retVal.encoded == -1) {
      throw EncodeException("XER", TypeName<T>::get(),
  ...
  virtual std::shared_ptr<T> decode(const std::vector<BYTE> buffer) {
  ...
  virtual std::shared_ptr<T> decode(const BYTE *buffer, int bufferLen) {
    T *msg = nullptr;
