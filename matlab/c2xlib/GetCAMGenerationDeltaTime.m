classdef GetCAMGenerationDeltaTime < matlab.System & coder.ExternalDependency
    ...
    methods (Access = protected)
        ...
        function [GenerationDeltaTime] = stepImpl(obj, StationID) 
            if coder.target('Rtw') || coder.target('Sfun') 
                err = int32(0);
                err = coder.ceval('getCAMGenerationDeltaTime', StationID, coder.wref(GenerationDeltaTime));
                printErrorCode(err);
            end            
        end
   ...
   elseif (err == -2)
    ...
    methods (Access = protected)
        function [GenerationDeltaTime] = getOutputSizeImpl(obj)
            GenerationDeltaTime = [1, 1];

        end 
        
        function [GenerationDeltaTime] = isOutputFixedSizeImpl(obj)
            true;

        end
        
        function [GenerationDeltaTime] = getOutputDataTypeImpl(obj)
            'int32';

        end
        
        function [GenerationDeltaTime] = isOutputComplexImpl(obj)
            false;
