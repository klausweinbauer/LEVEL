classdef GetCAMBasicVehicleContainerHighFrequencySteeringWheelAngle < matlab.System & coder.ExternalDependency
   ...
   elseif (err == -2)
    ...
    methods (Access = protected)
        function [SteeringWheelAngleValue, SteeringWheelAngleConfidence] = getOutputSizeImpl(obj)
            SteeringWheelAngleValue = [1, 1];
SteeringWheelAngleConfidence = [1, 1];

        end 
        
        function [SteeringWheelAngleValue, SteeringWheelAngleConfidence] = isOutputFixedSizeImpl(obj)
            true;
true;

        end
        
        function [SteeringWheelAngleValue, SteeringWheelAngleConfidence] = getOutputDataTypeImpl(obj)
            'int32';
'int32';

        end
        
        function [SteeringWheelAngleValue, SteeringWheelAngleConfidence] = isOutputComplexImpl(obj)
            false;
false;
