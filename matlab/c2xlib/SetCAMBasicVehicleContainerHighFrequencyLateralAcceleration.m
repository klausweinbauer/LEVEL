classdef SetCAMBasicVehicleContainerHighFrequencyLateralAcceleration < matlab.System & coder.ExternalDependency
    ...
    methods (Access = protected)
        ...
        function [] = stepImpl(obj, StationID, LateralAccelerationValue, LateralAccelerationConfidence) 
            if coder.target('Rtw') || coder.target('Sfun') 
                err = int32(0);
                err = coder.ceval('setCAMBasicVehicleContainerHighFrequencyLateralAcceleration', StationID, LateralAccelerationValue, LateralAccelerationConfidence);
                printErrorCode(err);
            end            
        end
   ...
   elseif (err == -2)
    ...
    methods (Static)
        function bName = getDescriptiveName(~)
            bName = 'SetCAMBasicVehicleContainerHighFrequencyLateralAcceleration';
        end
