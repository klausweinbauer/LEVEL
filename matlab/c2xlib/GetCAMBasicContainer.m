    ...
    methods (Access = protected)
        ...
        function [StationType, Latitude, Longitude, ConfidenceMajor, ConfidenceMinor, ConfidenceMajorOrientation, AltitudeValue, AltitudeConfidence] = stepImpl(obj, StationID) 
            if coder.target('Rtw') || coder.target('Sfun') 
                coder.ceval('getCAMBasicContainer', StationID, coder.wref(StationType), coder.wref(Latitude), coder.wref(Longitude), coder.wref(ConfidenceMajor), coder.wref(ConfidenceMinor), coder.wref(ConfidenceMajorOrientation), coder.wref(AltitudeValue), coder.wref(AltitudeConfidence));
            end            
        end
